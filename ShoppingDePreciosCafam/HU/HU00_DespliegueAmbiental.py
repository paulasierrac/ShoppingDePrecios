...
def hu00_despliegue_ambiental() -> tuple:
    ...
    try:
        ...
        url_base = out_config.get(_KEY_URL, "https://www.drogueriascafam.com.co/index.php?controller=search#2fce/fullscreen/m=f&q=")
        out_config["UrlCafam"] = url_base + "REEMPLAZAR"

        out_config.setdefault("NombreIniciativaCafam", _NOMBRE_INICIATIVA)
        out_config.setdefault("DrogueriaCafam",        _DROGUERIA)
        out_config.setdefault("NombreHojaResultado",   "ReportePricingCafam")
        ...
        out_config.setdefault("LoteCafam",                 "1000")
        out_config.setdefault("DelayCafam",                "300")
        ...
        if ultima_limpi != fecha_hoy:
            ...
            tabla_ex   = out_config.get("TablaCafam", "[Cafam]")
            conn   = conectar_bd(out_config)
            cursor = conn.cursor()
            cursor.execute(
