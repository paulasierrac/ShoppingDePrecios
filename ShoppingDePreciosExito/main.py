...
Modo debug/desarrollo:
  Activar con:  set RPA_DEBUG=true   (o en .env: RPA_DEBUG=true)
  Efectos:
    - Sin escrituras en BD (INSERT/UPDATE omitidos)
    - Chrome visible (no headless)
    - Sin envio de correos
    - Reporte Excel generado en ./debug/ local
    - Solo procesa los primeros 3 EANs del insumo
"""
...
TASK_NAME = "Main_ShoppingDePreciosExito"
...
def main() -> None:
    ...
    # ── Modo debug: inyectar flag en config y forzar Chrome visible ────────
    ...
    write_log(
        "Info",
        f"Inicia ejecucion del BOT ({io_config.get('NombreIniciativaExito', '')})",
        TASK_NAME, io_config
    )
    ...
    try:
        ...
        # ----------------------------------------------------------------
        p_aux_dic = {"$NombrePagina$": io_config.get("DrogueriaExito", "Exito")}
        ...
        write_log(
            "Info",
            f"Finaliza ejecucion del BOT ({io_config.get('NombreIniciativaExito', '')})",
            TASK_NAME, io_config
        )
        write_log("Info", "-" * 100, TASK_NAME, io_config)
        ...
        p_aux_dic_error = {
            "$NombrePagina$": io_config.get("DrogueriaExito", "Exito"),
            "$Proceso$":      io_config.get("NombreIniciativaExito", ""),
