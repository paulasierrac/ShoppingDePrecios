...
TASK_NAME = "Main_ShoppingDePreciosCruzVerde"
...
def main() -> None:
    ...
    write_log("Info",
              f"Inicia ejecucion del BOT ({io_config.get('NombreIniciativaCruzVerde', '')})",
              TASK_NAME, io_config)
    ...
    try:
        p_aux_dic    = {"$NombrePagina$": io_config.get("DrogueriaCruzVerde", "Cruz Verde")}
        ...
        write_log("Info",
                  f"Finaliza ejecucion del BOT ({io_config.get('NombreIniciativaCruzVerde', '')})",
                  TASK_NAME, io_config)
        write_log("Info", "-" * 100, TASK_NAME, io_config)
        ...
        p_aux_dic_error = {
            "$NombrePagina$": io_config.get("DrogueriaCruzVerde", "Cruz Verde"),
            "$Proceso$":      io_config.get("NombreIniciativaCruzVerde", ""),
